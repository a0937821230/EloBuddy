...
namespace AutoBuddy
{
    internal static class Program
    {
        ...
        private static void Loading_OnLoadingComplete(EventArgs args)
        {
            ...
            Chat.Print("AutoBuddy: Starting in 5 seconds5.");
            Core.DelayAction(Start, 5000);
            menu = MainMenu.AddMenu("AUTOBUDDY", "AB");
            menu.Add("sep1", new Separator(1));
            CheckBox c =
                new CheckBox("中路打燈，但如果有玩家在中路會自動換路(只在[自動]中套用)", true);
            ...
            string[] lanes =
            {
                "", "選擇路線: 自動", "選擇路線: 上路(Top)", "選擇路線: 中路(Mid)",
                "選擇路線: 下路(Bot)"
            };
            ...
            menu.Add("reselectlane", new CheckBox("該路有人，則重新選路線", false));
            menu.Add("disablepings", new CheckBox("不打信號", true));
            menu.Add("disablechat", new CheckBox("不在聊天中說話", true));
            CheckBox newpf = new CheckBox("自動循路", true);
            menu.Add("newPF", newpf);
            Slider hpValue = menu.Add("HPPot", new Slider("血量小於 {0}% 使用血罐", 43, 1, 100));
            ...
            menu.AddLabel("節省 RAM 資源");
            CheckBox noTextures = new CheckBox("移除遊戲中材質，重啟 LoL 套用");
            menu.Add("noTextures", noTextures);
            noTextures.OnValueChange += noTextures_OnValueChange;
            menu.Add("sep2", new Separator(10));
            menu.AddLabel("角色跟隨滑鼠 如果你在掛機別打開此功能");
            menu.Add("disableAutoBuddy", new CheckBox("停用 AutoBuddy 移動，F5 後套用", false));
            menu.AddSeparator(5);
            CheckBox autoclose = new CheckBox("當遊戲結束時自動關閉遊戲，F5 後套用", false);
            property2.GetSetMethod(true).Invoke(autoclose, new object[] { new Vector2(500, 20) });
            menu.Add("autoclose", autoclose);
            menu.AddSeparator(5);
            menu.Add("oldWalk", new CheckBox("使用舊版 orbwalking，F5 後套用", false));
            menu.Add("debuginfo", new CheckBox("顯示除錯資訊，F5 後套用", true));
