...
namespace AutoBuddy.Utilities.AutoLvl
{
    ...
    internal class CustomLvlSeq
    {
        ...
        public CustomLvlSeq(Menu m, AIHeroClient champ, string dir, string seq = "", int maxlvl = 18)
        {
            ...
            this.dir = dir;
            se = seq;
            Menu menuSettings = m.AddSubMenu("技能升級", "AB_SL_SETTINGS");
            CheckBox enabled = new CheckBox("啟用", true);
            menuSettings.AddGroupLabel("一般");
            menuSettings.Add(champ + "啟用", enabled);
            menuSettings.AddSeparator(10);
            menuSettings.AddGroupLabel("當前配置");
            profile1 = new CheckBox("設置 1", true);
            profile2 = new CheckBox("設置 2", false);
            menuSettings.Add(champ.ChampionName + Game.MapId + "p1", profile1);
            menuSettings.Add(champ.ChampionName + Game.MapId + "p2", profile2);


            
            updater = new CheckBox("更新默認排序");
            clear = new CheckBox("清除當前配置", false);
            menuSettings.Add("清除", clear);
            defau = new CheckBox("儲存當前配置為默認");
            menuSettings.Add("默認", defau);
            menuSettings.AddSeparator(10);

            menuSettings.AddGroupLabel("人性化");
            humanMin = new Slider("升等後至少延遲 {0} 毫秒後升級技能", 300, 0, 2000);
            humanMax = new Slider("升等後最長延遲 {0} 毫秒後升級技能", 500, 0, 2000);
            ...
            menuSettings.AddSeparator(10);
            menuSettings.AddGroupLabel("更新");
            menuSettings.Add("升級技能", updater);
            ...
            maxLvl = maxlvl;
            Menu menu = m.AddSubMenu("技能升級順序: " + champ.ChampionName);
